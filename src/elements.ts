...
export function getCurrentPageTitle(): string | null {
    ...
    if (titleNode) {
        const formattedText = titleNode.querySelector("yt-formatted-string.ytd-watch-metadata, .slim-video-information-title .yt-core-attributed-string:not(.cbCustomTitle), .watch-title-text-container") as HTMLElement;
        if (formattedText) {
